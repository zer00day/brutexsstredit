...
banner = """                                                                                       
 ...
 BruteXSS - Kaba Kuvvet Saldiri Programi
 
 http://blinkcursor.org
 
 XSS Açıgı Sömürme Araci Türkçe Sürümü

 Not: Hazir Wordlist 12 Payload İçerir.Daha Olumlu
 Sonuçlar İçin Kendi Wordlistinizi Ekleyiniz!
"""
def brutexss():
	if os.name == 'nt':
		os.system('cls')
	...
	print banner
	def again():
		inp = raw_input("[?] [Ç]ık yada [T]ekrar Dene? (ç/t)").lower()
		if inp == 't':
			brutexss()
		elif inp == 'ç':
			exit()
	...
	def wordlistimport(file,lst):
		try:
			with open(file,'r') as f: #Importing Payloads from specified wordlist.
				print(Style.DIM+Fore.WHITE+"[+] Payloadlar yükleniyor..."+Style.RESET_ALL)
				for line in f:
					final = str(line.replace("\n",""))
					lst.append(final)
		except IOError:
			print(Style.BRIGHT+Fore.RED+"[!] Wordlist bulunamadı!"+Style.RESET_ALL)
			again()
	def bg(p,status):
		try:
			b = ""
			...
			print(Style.BRIGHT+Fore.RED+"[!] Parametreler Url İçin Geçerli Değil!"+Style.RESET_ALL)
			again()
	def complete(p,r,c,d):
		print("[+] Saldırı Tamamlandı!")
		if c == 0:
			print("[+] Verilen Parametreler "+Style.BRIGHT+Fore.GREEN+"açıklı değil"+Style.RESET_ALL+" XXS için.")
		elif c ==1:
			print("[+] %s Parametre  "+Style.BRIGHT+Fore.RED+"açıklı"+Style.RESET_ALL+" XSS için .")%c
		else:
			print("[+] %s Parametreler "+Style.BRIGHT+Fore.RED+"açıklı"+Style.RESET_ALL+"  XSS için .")%c
		print("[+] Tarama Sonucu %s:")%d
		print bg(p,r)
		again()
	def GET():
			try:
				try:
					...
					print (Style.DIM+Fore.WHITE+"[+] Kontrol Ediliyor "+domain+" müsait..."+Style.RESET_ALL)
					connection = httplib.HTTPConnection(domain)
					connection.connect()
					print("[+] "+Fore.GREEN+domain+" uygun!"+Style.RESET_ALL)
					url = site
					paraname = []
					paravalue = []
					wordlist = raw_input("[?] Wordlist Dizini Gir (enterlarsan hazır wordlist yüklenir wordlist.txt)\n[?] > ")
					if len(wordlist) == 0:
						wordlist = 'wordlist.txt'
						print(grey+"[+] Hazır wordlist uygulanıyor..."+Style.RESET_ALL)
					else:
						pass
					payloads = []
					...
					grey = Style.DIM+Fore.WHITE
					print(Style.DIM+Fore.WHITE+"[+] "+lop+" Payloadlar yüklendi..."+Style.RESET_ALL)
					...
					for pn, pv in zip(paraname,paravalue): #Scanning the parameter.
						print(grey+"[+] Test ediliyor '"+pn+"' parametre..."+Style.RESET_ALL)
						fpar.append(str(pn))
						for x in payloads: #
							...
							if validate == "":
								...
								sys.stdout.write("\r[+] %i / %s payloadlar uygulandı..."% (progress,len(payloads)))
								sys.stdout.flush()
								progress = progress + 1
								...
								if x in sourcecode:
									...
									c = 0
						if c == 0:
							print(Style.BRIGHT+Fore.GREEN+"\n[+]"+Style.RESET_ALL+Style.DIM+Fore.WHITE+" '%s' parametre açıklı değil."+Style.RESET_ALL)%pn
							fresult.append("Açıklı değil")
							progress = progress + 1
							pass
						progress = 0
					complete(fpar,fresult,total,domain)
				except(httplib.HTTPResponse, socket.error) as Exit:
					print(Style.BRIGHT+Fore.RED+"[!] Site : "+domain+" kapalı!"+Style.RESET_ALL)
					again()
		...
		try:
			try:
				try:
					...
					path = urlparse.urlparse(site).scheme+"://"+urlparse.urlparse(site).netloc+urlparse.urlparse(site).path
					url = site
					param = str(raw_input("[?] Gönderilcek Veri: > "))
					wordlist = raw_input("[?] Wl Dizini Gir (enterlarsan hazır wl uygulanır wordlist.txt)\n[?] > ")
					if len(wordlist) == 0:
						wordlist = 'wordlist.txt'
						print("[+] Hazır wordlist kullanılıyor...")
					else:
						pass
					payloads = []
					wordlistimport(wordlist,payloads)
			...
			print(Style.BRIGHT+Fore.RED+"\n[!] HTTP HATASI! %s %s"+Style.RESET_ALL)%(e.code,e.reason)
	try:
		methodselect = raw_input("[?] Metod Seç:[G]ET yada [P]OST (G/P): ").lower()
		if methodselect == 'g':
			GET()
		elif methodselect == 'p':
			POST()
		else:
			print("[!] hatalı metod seçildi!")
			again()
	except(KeyboardInterrupt) as Exit:
		print("\nÇıkılıyor...")
